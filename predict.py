...
class Predictor:
    ...
    def predict(self, data):
        ...
        font = ImageFont.truetype("/usr/share/fonts/truetype/croscore/Cousine-Regular.ttf", size=18)
        ...
        for d in tqdm(data, desc="Predict"):
            ...
            for (top, right, bottom, left), pred in zip(d['locations'], preds):
                ...
                # draw a box
                draw.rectangle(((left, top), (right, bottom)), outline=colors[i%len(colors)])
                # draw a label
                text_width, text_height = draw.textsize(name)
                draw.rectangle(((left, bottom - text_height - 10), (right, bottom)), fill=colors[i%len(colors)], outline=colors[i%len(colors)])
